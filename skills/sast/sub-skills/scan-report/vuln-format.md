...
> **[필수] 필드 형식**: `**필드명**: 값` 형식 (줄 시작이 `**`). `- **필드명**: 값` 리스트 형식 금지 — validate_report.py가 `^- ` 접두사를 포함한 줄은 **ID**로 인식하지 않아 요약 테이블 링크가 "—"로 깨짐.
> **[필수] 헤딩 레벨**: 취약점 번호 헤딩은 `####` (h4). `###` (h3)는 스캐너 섹션 헤딩 전용.
...
> **[필수] 필드 형식**: `**필드명**: 값` (줄 시작이 `**`). `- **필드명**: 값` 리스트 형식 금지.
> **[필수] 헤딩 레벨**: `####` (h4) 사용. `###` 금지.
...
- **[필수] edge label에는 괄호 `(`/`)`를 쓰지 않는다.** edge label은 파이프 안이라 따옴표로 감쌀 수 없어, 괄호가 그대로 노출되면 mermaid 11.x 파서가 syntax error를 낸다(흐름도 전체가 깨짐). 괄호로 부연하고 싶으면 **노드 라벨**(따옴표로 감싸므로 괄호 안전)로 빼거나, edge label에서는 `·`로 대체한다. 예: `|동적 확인 · IP 게이트(위변조 가능)|` → `|동적 확인 · IP 게이트·위변조 가능|`. (`validate_report.py`가 edge label 괄호를 경고한다.)
...
| # | ID | 제목 | 상태 | 인증경계 |
|---|-----|------|------|:--------:|
| 1 | XSS-1 | callback 파라미터 XSS | 확인됨 | 외부망.무인증 |
| 2 | OAUTH-1 | OAuth state 검증 미흡 | 후보 | 외부망.인증 |
| ... | ... | ... | ... | ... |
