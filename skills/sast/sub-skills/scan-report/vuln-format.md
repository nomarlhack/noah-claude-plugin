...
| 진입 도메인 | base path | 표면 | 자격증명 | 신원 출처 | 인증 근거 | 도달성 |
|------------|-----------|------|---------|----------|----------|--------|
| ... | ... | ... | ... | 토큰/요청바디/요청헤더/내장자격증명/상호TLS·인증서/없음/미상 | 코드 근거/범위 밖 | 동적 확인 필요/확정 |

(AUTH_BOUNDARY 기반. **표는 도메인·표면이 1개여도 항상** 작성한다. **흐름도는 표면 또는 진입 도메인이 2개 이상일 때만** 작성한다 — 단일 표면이면 도달성 그래프가 자명하므로 표로 충분(빈 흐름도 금지). 흐름도는 도메인→표면 도달성을 실선=확정·점선=동적 확인 필요·주석=범위 밖으로 표기한다. POC 호스트는 `도달성=확정`인 도메인만 인라인하고, 그 외(`동적 확인 필요`·진입 도메인 `미상`)는 플레이스홀더로 둔다. **`도달성=확정`인 진입 도메인은 모두 개요 `**테스트 환경**` 필드에 포함한다**(validate_report.py 호스트 일관성 검증과 정합).)
