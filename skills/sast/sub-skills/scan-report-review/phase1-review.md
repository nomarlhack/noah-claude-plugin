...
**반환은 아래 카운트 + ID 목록만. 판정 근거·코드 인용은 eval MD와 master-list.json에 이미 있으니 반복하지 않는다(반환은 메인 컨텍스트에 누적됨).**
...
DISCARD: <ID 목록만> (사유는 master-list.json/eval MD)
OVERRIDE: <ID 목록만>
일관성 이상: <있으면 ID 목록, 없으면 "없음">
```
