...
```json
{
  ...
  "tag": null | "도구 한계" | "정보 부족" | "환경 제한" | "차단" | "동적 분석 생략" | "LLM endpoint 미확보" | "LLM endpoint 확인됨" | "LLM endpoint 정적 식별",
...
| candidate | LLM endpoint 미확보 | evidence_summary (`endpoint_unverified — <상세 사유>`) | `prereq_group == "llm"` 후보 한정 (스캐너 멤버십은 phase1.md frontmatter). SKILL.md Step 8-3 그룹 사전 단계가 endpoint를 확보하지 못한 경우 메인 에이전트가 placeholder phase2 결과 파일을 생성하고, `phase2-review`가 이를 인식해 본 tag를 부여한다. verified_defense 기록 금지(안전 판정 아님). |
