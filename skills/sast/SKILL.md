---
name: sast
description: "55개 취약점 스캐너 + AI 자율 탐색을 실행하고 결과를 통합 보고서로 작성하는 스킬. XSS, SSRF, SQLi, Code Injection, CSRF, TLS, 비즈니스 로직, LLM 취약점 등 모든 취약점 유형을 소스코드 분석, AI 자율 탐색, 동적 테스트로 점검한다. 사용자가 'noah-8719:sast', 'sast', '소스코드 취약점 스캔' 등을 요청할 때 이 스킬을 사용한다."
---

# Noah SAST — 통합 취약점 스캐너

55개 개별 취약점 스캐너와 AI 자율 탐색을 실행하고, 모든 결과를 하나의 통합 보고서로 작성하는 스킬이다.
...
<NOAH_SAST_DIR>/                   ← ${CLAUDE_PLUGIN_ROOT}/skills/sast
  ...
  prompts/                         ← 서브 에이전트 지시 문서
    ...
    guidelines-phase2.md           ← Phase 2 공통 지침 (LLM 그룹 전용 지침 12 포함)
  ...
  tools/                           ← Python 유틸리티 스크립트
    ...
    llm_channel_probe.py       ← LLM 그룹 채널 어댑터 (Step 8-3 / 8-4 LLM 한정)
  sub-skills/                      ← 내부 서브스킬
    scan-report/
    scan-report-review/
    chain-analysis/
...
| process-execution | command-injection, code-injection, ssti |
...
1. 아래 명령으로 샤딩한다. 인벤토리가 외부 파일로 분리되어 있으면 `--idor-source`에 idor-scanner.md를 별도 지정해야 기존 ID와 충돌하지 않는다.
   ```bash
   # 인벤토리가 idor-scanner.md 내 임베드인 경우
   python3 <NOAH_SAST_DIR>/tools/idor_shard.py <PHASE1_RESULTS_DIR>/idor-scanner.md \
     --rows-per-shard 60 --max-shards 18 --out-dir <SHARD_DIR>

   # 인벤토리가 외부 파일인 경우 (--idor-source 필수)
   python3 <NOAH_SAST_DIR>/tools/idor_shard.py <외부 인벤토리 파일> \
     --rows-per-shard 60 --max-shards 18 --out-dir <SHARD_DIR> \
     --idor-source <PHASE1_RESULTS_DIR>/idor-scanner.md
   ```
   출력: `idor_shard_{1..K}.md` + `idor_shards_manifest.json`. manifest의 각 샤드 항목에 **ID 채번 범위**(`id_start`, `id_end`)가 포함된다.

2. manifest를 Read해 각 샤드의 id_start/id_end를 파악한다:
   ```bash
   python3 -c "import json; m=json.load(open('<SHARD_DIR>/idor_shards_manifest.json')); \
...
**결정 2 — 동적 공격 페이로드 테스트 동의 여부 (`ATTACK_CONSENT`)**
4. 사용자가 동적 테스트 동의 / 거부 / 일부 거부(예: "공격 페이로드는 보내지 마, 보고서만") 의사를 표명.
...
2. Agent 도구로 사전 단계 에이전트를 디스패치한다. 변수는 resolve된 실제 경로 문자열로 치환:
   ...
   probe_mode: <full | connectivity-only | static-only>   ← 매트릭스에서 결정
   세션 정보: <SESSION_INFO>   ← URL_PROVIDED=N이면 빈 값
   sandbox 도메인: <SANDBOX_DOMAIN>   ← URL_PROVIDED=N이면 빈 값
   ```

3. 에이전트 반환 후 `<LLM_PROBE_DIR>/llm_endpoint.json`을 Read하여 `endpoints` 배열을 확인한다. `static-only` 모드 산출물은 각 endpoint에 `verified: false`, `static_only: true` 메타가 포함되어 있어야 한다.

**성공/실패 처리:**

- `endpoints`가 1개 이상 확정되고 `probe_mode == "full"`이면 LLM 그룹의 Step 8-4(Phase 2) 진입. 4개 LLM 스캐너 Phase 2 에이전트 프롬프트에는 `LLM_PROBE_DIR` 변수와 `llm_endpoint.json` 경로를 함께 전달한다. 다중 endpoint면 각 endpoint에 대해 Phase 2를 반복한다.
- `probe_mode == "connectivity-only"` 또는 `"static-only"`이고 `endpoints`가 1개 이상이면 Step 8-4는 skip하되, LLM 4개 스캐너에 대해 매트릭스가 지정한 tag로 placeholder phase2 결과 파일을 생성한다(아래 placeholder 형식의 `evidence.observations`에 모드별 사유를 기록).
- `endpoints`가 비어 있거나 `probe_mode != "full"`인 경우 LLM 그룹의 Phase 2(공격 페이로드 시도)를 skip한다. 메인 에이전트는 4개 LLM 스캐너 각각에 대해 다음 형식의 placeholder Phase 2 결과 파일을 생성하여 `phase2-review`가 본 후보들을 인식하도록 한다.

  파일 경로: `<PHASE1_RESULTS_DIR>/<llm-scanner>-phase2.md`

  `evidence.observations[0]`은 다음 marker 중 하나로 시작해야 한다 (`phase2-review`가 이 marker로 tag를 분기 부여):
    ...
    "results": [
      {
        ...
        "evidence": {
          "observations": ["<위 marker 형식>", "<endpoint 명세 발췌 또는 정적 단서 요약>"]
        }
      }
    ]
  }
  ...
  비-LLM 스캐너의 Phase 2는 본 실패/skip의 영향을 받지 않고 정상 진행한다.
...
- 각 스캐너의 분석 방법론은 해당 스캐너의 phase1.md(정적 분석 규칙) / phase2.md(동적 검증 cheat sheet)를 따른다. noah-sast는 중복 정의하지 않는다.
