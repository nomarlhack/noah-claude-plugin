...
4. **LLM 그룹 사전 단계 안내 (LLM 그룹이 활성인 경우만)** — 위 1·2의 sandbox URL과 세션은 Step 8-3 chat endpoint probe 및 LLM 스캐너 Phase 2의 헬퍼 호출에 그대로 재사용된다. chat endpoint 호스트가 사용자 제공 base URL과 다를 수 있음을 사용자에게 안내한다(예: HTTP API는 `api.sandbox`이지만 chat WS는 `chat.sandbox`/별도 게이트웨이). 별도 호스트는 probe-agent가 정적 분석으로 추출하므로 사용자 추가 입력 없이 처리된다.
...
- `endpoints`가 비어 있으면 LLM 그룹의 Phase 2를 skip한다. 메인 에이전트는 4개 LLM 스캐너 각각에 대해 다음 형식의 placeholder Phase 2 결과 파일을 생성하여 `phase2-review`가 본 후보들을 인식하도록 한다. (`scan-report-review/_contracts.md §3` tag enum의 `"동적 검증 불가(LLM endpoint 미확보)"` 케이스)
  ...
  Step 8-3 그룹 사전 단계가 chat endpoint를 확보하지 못하여 본 스캐너의 동적 검증을 수행하지 못함.
  사유: <probe-agent 반환의 종합 실패 사유 한 줄>
    ...
    "results": [
      {
        ...
        "evidence": {
          "observations": ["endpoint_unverified — chat endpoint not reachable", "<probe-agent 반환의 종합 실패 사유 한 줄>"]
        }
      }
    ]
  }
  ...
  각 후보 ID마다 동일한 `evidence.observations`를 반복한다. `phase2-review`가 본 파일을 읽고 4단계에서 `status: candidate` + `tag: "동적 검증 불가(LLM endpoint 미확보)"`를 일괄 부여한다(자세한 분기는 `sub-skills/scan-report-review/phase2-review.md §Step 진행 4` 참조).
