...
| **presence** (값 자체가 위험) | hardcoded-secrets, log-injection | 값/패턴의 존재 자체가 취약점. Source 도달성(O2) 불필요 — 코드에 박힌 비밀값(hardcoded-secrets)이나 민감 정보가 로그 sink에 도달하는 흐름(log-injection)이면 O4(sink 의미론 정합) 확인 후 후보 등록. 환경 변수 참조나 개행 제거는 안전. |
...
> **요약**: 안전 단정에는 의무 이행(증거)이 필요하고, 의심·이행불가·unsound 경계는 모두 후보 유지로 귀결된다. 능력형 토큰은 동질성을 입증하지 않는 한 클래스로 뭉개지 않는다.
...
**적용 예** (전 스캐너):
- IDOR: 형제 엔드포인트 다수가 소유권 게이트 보유, 이 메서드만 누락 → `IDOR_ASYMMETRIC` (이미 자동 후보). booking IDOR-1(COMPANION 분기만 멤버십 미검증)이 실측 사례.
...
- **같은 권장 조치 + 같은 sink 함수** = 1 후보로 묶고, 본문 `### Code`에 모든 file:line 열거
- 같은 라이브러리/프레임워크의 동일 sink가 여러 호출부에서 반복되면 통합
- 예: 모든 admin 엑셀 export의 `setCellValue` 동일 결함 → 1 후보, file:line 10개 열거
