...
#### 6-A-3: 파일 단위 disposition (결정/방어 스캐너 필수)
...
**규칙**: 이 스캐너들은 **인덱스가 매치한 *서버 결정 파일*을 클래스로 뭉개지 못한다.** 각 서버 결정 파일은 결과 MD에 **파일명을 적고 개별 disposition**(후보 / 안전 / 무관 + 근거 1줄)해야 한다. "서버 결정 파일"은 경로에 `controller·service·router·checker·handler·middleware·guard` 또는 `server·pages/api·app/api·api`가 포함된 `.ts/.js`(테스트·`.d.ts`·클라이언트 컴포넌트 제외)다. *경로 판별은 비용 배분용 휴리스틱일 뿐 안전 판정 기준이 아니다 — 누락된 파일도 source-first(§6-B)에서 별도로 다룬다.*

- 비용은 **매치 수가 아니라 파일 수**다. 클라이언트 컴포넌트·문서·테스트는 대상이 아니므로 부담은 작다.
...
- `phase1_review_assert.py`가 locindex의 서버 결정 파일 집합 ∩ MD 미명시를 기계 검증한다(`§6-A-3` 위반 시 exit 7). 파일명이 MD에 등장만 해도(후보·안전·무관 어느 판정이든) 충족 — 막는 것은 "이름조차 안 적고 묶어서 사라지는 것"이다.
