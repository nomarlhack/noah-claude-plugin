...
```
<!-- FILE_DISPOSITION files=<총파일수> accounted=<명시한파일수> method="<처리 방식 한 줄>" -->
```
...
- `phase1_review_assert.py`가 locindex의 distinct 파일 집합 ∩ MD 미명시를 기계 검증한다(`§6-A-3` 위반 시 exit 7). 파일명(basename)이 MD에 등장만 해도(후보·안전·무관 어느 판정이든) 충족 — 막는 것은 "이름조차 안 적고 묶어서 사라지는 것"이다.
