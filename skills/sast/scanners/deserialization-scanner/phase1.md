...
본 스캐너는 `exclusion_policy: capability` — 매치가 곧 역직렬화 능력의 실재를 뜻한다(decision-framework §2-D). 능력형 토큰(위 "Sink 의미론" 표: `ObjectInputStream.readObject`/`pickle.loads`/`unserialize`/`Marshal.load`/`yaml.load`(unsafe)/`XStream.fromXML`/`BinaryFormatter` 등) ast-tier 매치는 **클래스로 일괄 제외 금지**다.

ast-tier 능력형 매치는 하나도 빠짐없이 dispositioned되어야 한다. 각 매치는 (1) 후보 등록 / (2) 개별 SAFE(O4 증거: safe loader·화이트리스트·HMAC 검증 file:line 인용 / 입력이 trusted 내부 출처) / (3) 동질 하위클래스 SAFE(판별 축 명시 + spot-check) 중 하나로 처리. 비동질 클래스(같은 토큰이 안전·위험 양쪽) 일괄 제외 금지.

결과 MD에 마커 1줄 포함(게이트 `phase1_review_assert.py`가 파싱):

```
<!-- OBLIGATION ast_matches=<locindex ast 매치 수> dispositioned=<처리한 수> method="<처리 요약>" -->
```

`ast_matches`는 locindex `tier_counts.ast`와 같아야 하고(언더신고 불가), `dispositioned`가 `ast_matches`와 같아야 한다(잔여는 `[INCOMPLETE]`). generic 노이즈는 §6-A-2 COVERAGE로 처리.
