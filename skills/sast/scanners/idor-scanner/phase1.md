...
3. **인벤토리 도구는 두 모드 병행.** semgrep taint는 람다 클로저·DTO 필드·체이닝에서 추적 한계가 있다(엔진 무관 보편 한계). 그래서 dataflow 확정분(taint) + 컨트롤러 source-only 스캔(전수 안전망)을 합쳐야 FN 0에 가까워진다. 새 언어 컨트롤러 스캔 휴리스틱은 그 언어의 매핑·시그니처 문법에 맞게 추가한다(`idor_inventory.py` 확장).
   > **도구 한계 (현재)**: `idor_inventory.py` 컨트롤러 전수 스캔(모드 2)은 **Spring(Java/Kotlin)·FastAPI/Flask/Django(Python)·Express(Node.js)**를 지원한다. Spring·FastAPI는 데코레이터+시그니처로 라우트·입력을 정확히, Express는 라우트+경로변수 전수 열거+입력 best-effort 힌트(외부 핸들러는 입력 미상), Flask는 데코레이터 라우트+경로변수(`<int:id>`)+본문 `request.*` 힌트, Django는 `urls.py` 라우트+경로변수 열거(핸들러가 `views.py`라 입력은 거의 미상)로 처리한다. Rails·Go 등 나머지 호출/본문접근형은 taint 룰(모드 1)만 사용 가능하며, taint가 추적하지 못하는 흐름(DTO 필드·람다 바인딩 등)은 **에이전트가 source-first 패턴 탐색(Step 6-B-2)으로 직접 보완**해야 한다.
