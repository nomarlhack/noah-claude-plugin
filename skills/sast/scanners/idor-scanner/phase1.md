...
- **식별자는 이름이 아니라 역할로 판별**: 외부 입력이 "어떤 리소스/레코드/파일을 가리키는 값"이면 IDOR 식별자다. **판별 기준은 이름·명명 규칙이 아니라 역할 — "이 값이 달라지면 다른 사람의 리소스에 접근하게 되는가"**이다. 특정 명명 패턴으로 후보를 한정하면 놓친다. 외부 입력은 **프레임워크가 요청 데이터(경로·쿼리·바디·헤더·쿠키·폼·멀티파트 등)를 핸들러 인자에 바인딩하는 표지를 전수**(메커니즘 불문 — 어노테이션·데코레이터·파라미터 매핑·요청 객체 접근 등, 해당 프레임워크의 입력 바인딩 방식 전부) 식별한 뒤, 각 값이 리소스를 지목하는 역할인지(DB 조회 키, 파일/스토리지 경로, 토큰/핸들, 캐시 키, 외부 시스템 참조 등 형태 불문) 의미로 판정하라. **헤더·쿠키 source는 게이트웨이가 인증 후 덮어쓰는 신뢰 헤더와 구조적으로 구분 불가하므로**, 후보로 올린 뒤 게이트웨이 정책(해당 헤더명을 덮어쓰는지)을 확인하여 안전 분기를 처리한다(덮어쓰기 확인 못 하면 후보 유지가 안전).
- **[필수] 인증 자체가 없는 진입점도 IDOR 후보**: IDOR을 "인증된 사용자가 다른 사용자 리소스에 접근"으로만 좁히면 미탐. 컨트롤러가 외부 식별자(accountId/userId/이메일/전화번호 등 역할상 사용자 지목값)를 **세션이 아니라 클라이언트 입력에서** 받아 service에 그대로 전달하면, 인증 자체가 부재하든 인가만 누락이든 결과는 동일하다 — 임의 사용자 리소스 접근. 다음 신호를 만나면 후보로 올린다:
  - 핸들러 시그니처에 사용자 지목 입력(요청 헤더·쿼리·바디 등 입력 바인딩으로 받은 식별자류 값)이 있고, 컨트롤러/메서드/미들웨어·AOP 어느 계층에도 인가 게이트(소유권 비교·선언적 인가·세션 사용자 ID 사용)가 없음.
...
- **[필수] 분기 조건부 인가 — 검증 함수 본문을 확인하기 전에는 `[검증]` 금지**: 검증 함수에 전달된 클라이언트 입력이 **그 함수의 동작 경로를 가르는 분기 판별자**로 쓰이면(종류·모드·역할·상태·관계 등 형태 불문), 함수 본문을 Read하여 **모든 분기가 소유권을 강제하는지** 확인하기 전에는 `[검증]`으로 종결하지 않는다.
  - **세션·인증 신원이 검증 함수의 인자에 있다는 사실만으로 "검증됨"으로 단정하지 않는다(표면 신호 금지).** 신원을 넘긴 뒤 함수 내부에서 분기로 그 검증을 건너뛸 수 있다.
  - 분기 여부가 불확실하면 본문을 열어 확인하고, 열지 못하면 `[미확인]`(후보 유지)로 둔다.
  - 단, 클라이언트 입력이 분기를 만들지 않고 **단순 소유자 일치 비교 대상으로만** 쓰이면 본 의무는 발동하지 않으며 기존 판정 규칙을 따른다.
...
> **외부입력**(핸들러가 요청 데이터를 받는 입력 바인딩 표지 — **이름·타입 무관**, 바인딩 메커니즘이 표지) → **리소스접근**(repository/service 조회·조작 호출에 그 값 전달) → **소유권게이트 부재**(요청자 신원 vs 리소스 소유자 비교, 선언적 인가, 객체 인가 미들웨어/AOP가 처리 경로에 없음)
...
| 단계 | 조건 | 처리 |
|------|------|------|
| **① 자동 후보 (비대칭)** | 같은 컨트롤러/형제 엔드포인트 다수는 소유권 게이트(어노테이션/검증 호출)가 있는데 **이 메서드만 누락**; 또는 **동일 리소스 타입에 대해 소유자 결합 조회(요청자 식별자를 조회 인자로 받음)와 식별자 단독 조회가 공존하고 후자에 게이트 부재**(인증/비인증 자매 경로 비대칭) | master-list 후보 등록 (라벨 `IDOR_ASYMMETRIC`, 정밀도 높음) |
...
### 검토 인벤토리 출력 형식 (파일 단위 체크리스트)

**[필수] 인벤토리는 기계 생성한다.** 에이전트 수기로는 대량 진입점(수백~천여 건 규모)을 컨텍스트 한계로 누락한다. **taint 모드 + 컨트롤러 스캔 모드를 함께** 실행한다 — taint는 dataflow 확정 고신뢰분, 컨트롤러 스캔은 람다 클로저·DTO 우회·체이닝으로 taint flow가 닿지 못하는 진입점을 source-only로 메우는 안전망(FN 방지):
...
이 도구는 외부 입력 어노테이션(`@PathVariable`/`@RequestParam`/`@RequestBody`/`@RequestHeader`/`@CookieValue`/`@ModelAttribute`/`@RequestPart`) 7종으로 모든 진입점을 전수 추출하여 **진입점 파일별 `#### <파일>` 섹션 + 그 파일의 엔드포인트 체크리스트**(`확인 | 엔드포인트 | 외부입력 | 위치 | 출처 | 인증 | 소유권게이트([미확인])`)로 출력한다. 인증 미경유(`[제외]`)를 포함한 파일이 상단 정렬된다(우선 검토 — 위 "[미확인] 종결 금지" 규칙의 입력).

#### 대규모 인벤토리 샤딩 (deep-read 병렬화)
...
**[필수] 검토 단위(파일) ≠ 후보 등록 단위(진입점).** 파일 단위 검토는 *커버리지*용이지 후보를 파일당 1건으로 묶으라는 게 아니다. **게이트가 부재한 각 엔드포인트는 자기만의 file:line·url_path·상태·POC를 갖는 별도 후보**다(같은 파일·같은 컨트롤러여도, sink·수정 위치가 같아도). 진입점을 한 후보로 합치지 않는다 — 합치면 진입점별 상태·POC가 어긋나고, 한쪽만 동적 확인됐을 때 다른 진입점이 "확인됨"을 거짓 상속한다(decision-framework §5 불변식). 같은 컨트롤러의 서로 다른 엔드포인트(서로 다른 핸들러 메서드/route)는 서로 다른 진입점이므로 각각 후보다.

에이전트는 검토한 파일 섹션을 결과 파일에 포함하되, **각 엔드포인트의 소유권게이트 열을 다음 형식으로 채운다**(거짓 복사 방지):

- `[검증] <service>.<method>():<file:line>` — 완전 검증. **호출된 게이트 함수의 파일·라인을 반드시 인용**. 함수명만 적는 것 금지. **클라이언트 입력이 분기 판별자로 쓰이면**(§게이트 판정 "분기 조건부 인가" 참조), 게이트 호출부 file:line과 **별개로 검증 함수 본문에서 "부재 시 거부"가 모든 분기에 적용됨을 보인 file:line**을 함께 인용해야 `[검증]`이 성립한다. 다만 **어노테이션·AOP·인터셉터 등 선언적 인가**는 그 **선언 위치 file:line과 적용 범위 인용으로 갈음**한다. 호출 존재·인자 전달만으로는 `[검증]`이 아니다.
...
- **[필수] 인가가 판별자로 분기하면 전 분기 검증**: 인가 결정이 enum·role·type·플래그 등 판별자로 분기하면(switch/when/if-else), **모든 분기를 개별 검증**한다. 한 분기에서 게이트를 봤다고 다른 분기를 안전 판정하지 않는다. `[검증]`은 **모든 분기**가 게이트를 가질 때만 가능하며, 한 분기라도 부재/미확인이면 `[부분]: <그 분기>`로 후보 승격한다. 인증된(세션 식별자를 받는) 엔드포인트라도 인가가 분기 조건부면 이 의무가 적용된다.
- **[필수] fail-open 금지 — "부재 시 거부"를 입증해야 `[검증]`(입증책임 역전)**: 인가성 조회·게이트는 **부재 경로(빈 결과/null/no-match/미일치)에서 접근이 거부(throw/deny)됨을 본문 인용으로 입증**해야 `[검증]`이다. 입증 못 하거나 불명이면(조회만 하고 통과, 결과를 차단에 사용하지 않음 등) 형태 불문 후보로 유지한다. **판정은 코드 shape·메서드 네이밍·언어·관용구가 아니라 "부재 시 거부" 효과로 한다** — 특정 패턴(Optional 소비 방식 등)에 의존하지 않으므로, 어떤 프로젝트·어떤 조회 형태든 동일하게 적용된다. "조회 존재"를 "검증 존재"로 오인 금지.
...
- **[필수] session-override 매치는 전수 등록**: semgrep 룰 `noah-*-idor-session-identity-override`(클라이언트 입력 ↔ 세션 입력의 신원 폴백 — Kotlin elvis `?:`/Java 삼항·`Optional.orElse`)는 고정밀 IDOR 시그널이다. 이 룰의 **모든 매치를 후보로 등록**하거나, service Read 후에만 명시 폐기(사유 기재)하라 — 라벨만 `[미확인]`/`[적용]`로 두고 넘기는 조용한 누락 금지. `phase1_review_assert.py`가 미등록 매치를 차단한다(기계적 백스톱).
- **[필수] `implicit-bind`/`[제외]` 행은 file:line 증거 필수**: 인벤토리 외부입력 열의 `implicit-bind <Type>`은 어노테이션 없이 요청에서 바인딩되는 복합 타입(커맨드객체 DTO·enum)이다 — DTO 필드가 신원 식별자일 수 있으므로 해당 타입을 **Read해 필드를 확인**한다("세션 단독"으로 오판 금지). `auth=[제외]` 판정과 `소유권게이트` 결론은 라벨만 뒤집지 말고 근거 file:line을 인용한다.
...
**[필수] 인벤토리 행을 후보로 승격할 때 행(진입점) 1개 = 후보 1개다.** 같은 파일의 여러 행을 한 후보로 합치지 않는다(파일은 표현 단위일 뿐). 행마다 별도 후보로 등록하고, 각자의 url_path·소유권게이트 상태를 분리 기재한다.
...
**[필수] 표면 명칭 기반 격리 가정 금지**: 경로 prefix·명칭(내부·관리·파트너로 읽히는 것 포함)만으로 표면의 외부 도달 불가를 단정해 후보를 제외하지 않는다. "게이트웨이가 신원을 강제할 것"이라는 가정은 코드 근거 없이는 안전 근거가 아니다. 도달성을 뒷받침하는 진입 경로 근거(자기 외부 URL 구성 코드·ingress/게이트웨이 설정·정적 클라이언트 산출물)가 없으면, 제외하지 말고 도달성 동적 확인 필요 후보로 유지한다.
...
3. **인벤토리 도구는 두 모드 병행.** semgrep taint는 람다 클로저·DTO 필드·체이닝에서 추적 한계가 있다(엔진 무관 보편 한계). 그래서 dataflow 확정분(taint) + 컨트롤러 source-only 스캔(전수 안전망)을 합쳐야 FN 0에 가까워진다. 새 언어 컨트롤러 스캔 휴리스틱은 그 언어의 매핑·시그니처 문법에 맞게 추가한다(`idor_inventory.py` 확장).
   > **도구 한계 (현재)**: `idor_inventory.py` 컨트롤러 전수 스캔(모드 2)은 **Spring(Java/Kotlin)·FastAPI/Flask/Django(Python)·Express(Node.js)·Rails(Ruby)·Go(Gin/Echo/chi/net-http)**를 지원한다. Spring·FastAPI는 데코레이터+시그니처로 라우트·입력을 정확히, Express·Go는 라우트+경로변수 전수 열거+본문 입력 best-effort 힌트(외부 핸들러는 입력 미상), Flask는 데코레이터 라우트+경로변수(`<int:id>`)+본문 `request.*` 힌트, Django/Rails는 라우트 정의(`urls.py`/`routes.rb`)와 핸들러(`views.py`/컨트롤러)가 분리되어 라우트+경로변수 열거(입력은 거의 미상)로 처리한다. 그 외 마이너 프레임워크(Spring WebFlux·Ktor·Phoenix·Laravel 등)는 taint 룰(모드 1)만 사용 가능하며, taint가 추적하지 못하는 흐름(DTO 필드·람다 바인딩 등)은 **에이전트가 source-first 패턴 탐색(Step 6-B-2)으로 직접 보완**해야 한다.
