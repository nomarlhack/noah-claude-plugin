...
### WHITELABEL_SPEL_RISK — Whitelabel/에러 메시지 SpEL 평가·입력 반사 위험

Whitelabel(또는 커스텀) 에러 응답이 **사용자 입력값을 에러 메시지에 반사**하고, 그 값이 **SpEL로 평가되면 RCE**로 이어진다 (spring-boot#4763, CVE-2022-22963 류). Phase 1은 아래 정적 선행조건을 포착해 `WHITELABEL_SPEL_RISK` 후보로 등록하고, 실제 평가 여부는 Phase 2(`${7*7}` 페이로드)에서 확정한다.

판정에 필요한 두 축을 확인한다:

**(A) 취약 버전 신호** — 빌드 파일(`build.gradle(.kts)`/`pom.xml`)에서 버전 확인:

| 패턴 | 판정 |
|------|------|
| Spring Boot < 1.3.0 (whitelabel SpEL 평가, #4763 미수정) | 후보 (HIGH) |
| `spring-cloud-function-context` 의존 + CVE-2022-22963 영향 범위(3.1.x ≤ 3.1.6, 3.2.x ≤ 3.2.2) | 후보 (HIGH) |
| 그 외 최신 버전 | (A)만으로는 제외 — (B) 단독 평가 |

**(B) 입력 반사 설정 신호** — effective config에서 에러 응답에 사용자 입력을 노출하는 설정:

| 패턴 | 판정 |
|------|------|
| `server.error.include-message=always` | 후보 (입력값이 에러 메시지에 반사 → 반사형 XSS/SpEL 표면) |
| `server.error.include-binding-errors=always` | 후보 (바인딩 실패 시 입력값 반사) |
| 위 설정 없음 (기본값 `never`) + 최신 버전 | 제외 |

> **종합 판정**: (A) 취약 버전이면 버전 단독으로 후보(HIGH). (B) 입력 반사 설정만 있으면 후보(MEDIUM, 반사형 XSS 우선·SpEL은 Phase 2 확인). 둘 다 없고 Whitelabel만 기본 활성이면 `WHITELABEL_ENABLED`(정보 노출)로만 처리하고 본 항목은 제외.

> **주의(오탐 방지)**: 최신 Spring Boot(2.x/3.x)는 `getMessage()`에서 SpEL을 평가하지 않으므로, 버전 신호 없이 Whitelabel 노출만으로 SpEL RISK를 단정하지 않는다. 본 항목은 "Phase 2에서 `${7*7}` 평가를 시도할 가치가 있는지" 트리아지 용도다.
...
6. **Actuator `heapdump`/`threaddump` 노출**: exposure.include에 포함되면 heap/thread 상태 덤프 — 메모리 내 시크릿/토큰 노출.
7. **`spring.jpa.show-sql=true` 또는 `logging.level.org.hibernate.SQL=DEBUG`**: prod에서 SQL 전체 로깅 — 민감 데이터 로그 노출.
8. **Spring4Shell (CVE-2022-22965) 미패치**: Spring Framework 5.2.x, 5.3.x 구버전 + JDK 9+ + RequestMapping 객체 바인딩.
9. **Log4Shell (CVE-2021-44228) 잔존**: `log4j-core` 2.x 구버전 직접/전이 의존성.
10. **`server.servlet.session.tracking-modes`**: `URL` 포함 시 JSESSIONID가 URL에 노출 (쿠키 외).
11. **`spring.sql.init.*` data-locations이 사용자 입력 경로**: 초기화 스크립트 경로 변조 시 임의 SQL 실행.
