...
**어디서 쓰이는가**: 실제로는 거의 쓰이지 않는다. `locindex.json`이 있으면 항상 locindex를 우선 사용하고, 이 파일은 locindex가 없을 때의 fallback이다. 같은 디렉토리에 `locindex.json`이 함께 생성되므로 현실적으로 fallback이 발동하는 경우는 드물다.
