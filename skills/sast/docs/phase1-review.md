...
flowchart TD
    ...
    PICK --> BLIND["blind eval\nphase1_review_blind_read.py\n판정 섹션 마스킹"]
    BLIND --> READ["소스 파일 Read\n(마스킹된 뷰 기준 독립 판정)"]

    READ --> AX["5개 축 적용\n① 코드 스니펫 정확성\n② Source→Sink 흐름\n③ 부재 주장 검증\n④ Source 도달성\n⑤ 플랫폼 방어"]

    AX --> JUDGE{"판정"}
    JUDGE -->|"Source 도달 가능\n오류 없음"| CONFIRM["CONFIRM\nphase1_validated: true"]
    JUDGE -->|"오류 있으나\n후보는 유지"| OVERRIDE["OVERRIDE\nphase1_validated: true\n+ eval MD 수정 권고"]
    JUDGE -->|"Source 도달 불가\n또는 플랫폼이 이미 방어"| DISCARD["DISCARD\nstatus: safe\nsafe_category 기록"]

    CONFIRM --> EVAL["evaluation/<scanner>-eval.md 작성\n(Phase 2 이후 참조 기준)"]
    OVERRIDE --> EVAL
    DISCARD --> EVAL

    EVAL --> UPD["master-list.json 갱신\nphase1_validated: true"]
    UPD --> GATE["phase1_review_assert.py\n게이트 3종 검증"]

    GATE -->|"exit 0"| P2["Phase 2 진입"]
    GATE -->|"exit 1/7"| RETRY["phase1-review 재호출\n(최대 2회)"]
    RETRY --> PICK
```
