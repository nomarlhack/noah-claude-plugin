...
각 파일 안에는 후보 섹션, 커버리지 주석, 게이트 주석, MANIFEST 블록이 들어있다:
...
`phase1_review_assert.py`는 이 파일들을 읽어서 COVERAGE/OBLIGATION/FILE-PRESENCE 주석의 숫자를 locindex 실제값과 비교한다. 이후 `phase1-review` 에이전트는 원본 MD 대신 `evaluation/<scanner>-eval.md`(리뷰 평가본)를 생성하며, Phase 2와 보고서는 이 eval MD를 참조한다.

---

```
┌────────────┬────────────────────────────────────┬────────────────────────────────────────────────────┐
│   게이트   │   결과 MD에 작성하는 주석 형식      │              스크립트가 검증하는 것                │
├────────────┼────────────────────────────────────┼────────────────────────────────────────────────────┤
