...
세 게이트는 하나의 목적(침묵을 막는다)을 공유하지만, **스캐너 성격과 위험 지점이 다르기 때문에 적용 단위가 다르다**.

| 위험 지점 | 질문 | 적용 게이트 |
|-----------|------|------------|
| 매치 수가 많아 일부를 말없이 건너뜀 | "전체 매치를 설명했는가?" | COVERAGE |
| 고위험 패턴을 "어차피 대부분 노이즈" 클래스로 뭉갬 | "위험한 매치를 1건씩 봤는가?" | OBLIGATION |
| 파일 자체를 언급조차 하지 않고 넘어감 | "모든 파일이 이름이라도 올라왔는가?" | §6-A-3 |

각 게이트가 노리는 실패 패턴이 다르다.

**COVERAGE가 막는 패턴**: 에이전트가 1264건 중 일부를 조용히 누락한 경우.
```
<!-- COVERAGE matches=349 accounted=349 -->
```
실제 매치가 1264건인데 349건만 설명하면 FAIL. "나머지 915건은 어디 갔나"를 묻는다.

**OBLIGATION이 막는 패턴**: 고정밀 sink 룰 매치를 "어차피 false positive" 클래스로 일괄 제외한 경우.
```
exec() 계열 17건 → "전부 RegExp.exec() 오매치 클래스" (실제 읽지 않고)
```
COVERAGE처럼 숫자만 맞춰도 의미가 없다. sink 룰은 1건씩 소스를 Read하고 disposition해야 한다.

**§6-A-3이 막는 패턴**: 248개 파일 중 일부 파일명을 아예 MD에 올리지 않은 경우.
```
"React fetch 클라이언트 클래스 전부 제외" (파일명 나열 없이)
```
파일명이 없으면 어떤 파일을 건너뛰었는지 알 수가 없다.

세 게이트를 하나로 합칠 수 없는 이유도 여기 있다. COVERAGE는 매치 단위, OBLIGATION은 고정밀 룰 매치 단위, §6-A-3은 파일 단위로 측정 대상이 다르다. 고볼륨이 아닌 스캐너에 COVERAGE를 적용하면 비용 대비 의미가 없고, capability가 없는 스캐너에 OBLIGATION을 적용하면 과도한 부담이 된다.
