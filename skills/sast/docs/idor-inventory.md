...
- **controller-scan 모드** (`--project-root`): `.java`/`.kt`(Spring)·`.py`(FastAPI·Flask·Django)·`.js`/`.ts`(Express)·`.rb`(Rails) 컨트롤러/라우트에서 외부 식별자를 받는 진입점을 source-only로 전수 추출한다. taint flow가 엔진 한계(람다 클로저·체이닝·DTO 필드)로 닿지 못하는 진입점을 메우는 **안전망(FN 방지)**이다.
...
- **controller-scan은 Spring(Java/Kotlin)·FastAPI/Flask/Django(Python)·Express(Node.js)·Rails(Ruby)를 지원**한다. Spring·FastAPI는 데코레이터+시그니처에서 라우트·입력을 정확히 추출한다. Express는 본문접근형이라 **라우트+경로변수는 정확히 전수 열거하고 입력은 best-effort 힌트**(인라인 핸들러 본문 `req.*` 토큰), 외부 핸들러는 `<입력 미상 - 핸들러 Read>` 마커로 등재한다. Flask는 하이브리드 — 라우트는 데코레이터(`@app.route`/`@app.get`)·경로변수는 `<int:id>`로 정확히, 입력은 본문 `request.*` 토큰을 best-effort 힌트로 단다. Django는 라우트(`urls.py`)와 핸들러(`views.py`)가 분리되어 **라우트+경로변수만 열거하고 입력은 거의 미상**(`<입력 미상 - view Read>`)이다. Rails도 라우트(`routes.rb` DSL)와 핸들러(컨트롤러)가 분리되어 get/post 등 DSL의 경로+경로변수(`:id`)를 열거하고 입력은 미상(`<입력 미상 - controller Read>`)으로 처리한다(`resources` RESTful 자동은 미지원). Go는 아직 미지원이라 **taint 모드만** 동작하며, 그 갭은 에이전트 source-first 보완이 백스톱이다.
