...
라우트(`config/routes.rb` DSL)와 핸들러(컨트롤러)가 분리된 구조라 Django와 유사하다. `get`/`post`/`put`/`patch`/`delete`/`match` DSL의 경로 + 경로변수(`:id`, Express와 같은 형식, `EXPRESS_PATH_VAR_RE` 재사용)를 추출하고, 핸들러(`to: 'ctrl#action'`)가 `app/controllers`에 분리되어 경로변수가 없으면 `<입력 미상 - controller Read>`로 등재한다. `routes.draw`를 필수 마커로 사용(일반 `.rb`의 `get`/`post` 메서드 호출 오탐 방지). **한계**: `resources`(RESTful 자동 7액션)·블록 라우트·`scope`/`namespace` prefix는 미반영.
...
- **controller-scan은 Spring(Java/Kotlin)·FastAPI/Flask/Django(Python)·Express(Node.js)·Rails(Ruby)·Go(Gin/Echo/chi/net-http)를 지원**한다. Spring·FastAPI는 데코레이터+시그니처에서 라우트·입력을 정확히 추출한다. Express는 본문접근형이라 **라우트+경로변수는 정확히 전수 열거하고 입력은 best-effort 힌트**(인라인 핸들러 본문 `req.*` 토큰), 외부 핸들러는 `<입력 미상 - 핸들러 Read>` 마커로 등재한다. Flask는 하이브리드 — 라우트는 데코레이터(`@app.route`/`@app.get`)·경로변수는 `<int:id>`로 정확히, 입력은 본문 `request.*` 토큰을 best-effort 힌트로 단다. Django는 라우트(`urls.py`)와 핸들러(`views.py`)가 분리되어 **라우트+경로변수만 열거하고 입력은 거의 미상**(`<입력 미상 - view Read>`)이다. Rails도 라우트(`routes.rb` DSL)와 핸들러(컨트롤러)가 분리되어 get/post 등 DSL의 경로+경로변수(`:id`)를 열거하고 입력은 미상(`<입력 미상 - controller Read>`)으로 처리한다(`resources` RESTful 자동은 미지원). Go는 본문접근형(Express 유사)이라 Gin/Echo `.GET`·chi `.Get`·net-http `.HandleFunc` 라우트+경로변수(`:id`/`{id}`)를 열거하고 본문 `c.Param`/`c.Query`/`mux.Vars` 토큰을 입력 힌트로 단다(외부 핸들러는 미상). 그 외 마이너 프레임워크(Spring WebFlux·Ktor·Phoenix·Laravel 등)는 **taint 모드만** 동작하며, 그 갭은 에이전트 source-first 보완이 백스톱이다.
