...
IDOR 점검 대상 진입점(외부 식별자를 받아 리소스에 접근하는 엔드포인트)을 **기계적으로 전수 수집**하는 인벤토리 생성기다. 취약/안전을 **판정하지 않는다** — "빠짐없이 목록화"가 유일한 책임이며, 소유권게이트 판정(✓/❌)은 에이전트가 service Read로 채운다.
...
| 한다 | 안 한다 |
|------|---------|
| 외부 입력 어노테이션을 가진 모든 진입점 전수 추출 | 취약/안전 판정 (소유권게이트는 `❓`로 초기화) |
...
`❓ 미확인` 컬럼을 채워 ✓(게이트+범위 적정)/❌(부재)/⚠(부분)로 승격하는 것은 **에이전트가 코드를 직접 Read**해서 수행한다. 도구는 "여기 봐야 한다"까지만 책임진다.
...
| **인증** | `제외`=인증 게이트 미경유(우선 검토) / `적용`=그 외(단정 아님) / `—`=미발견·미지원 |
| 소유권게이트 | **항상 `❓ 미확인`으로 초기화** — 에이전트가 service Read 후 채움 |

> **에이전트가 소유권게이트를 채우는 규칙** (phase1.md):
> - `✓ <service>.<method>():<file:line>` — 완전 검증. 게이트 함수의 파일·라인 인용 필수.
> - `❌ 부재` — service Read 후 게이트 없음 확정.
> - `⚠ 부분: <우회 경로>` — 부분 게이트(타입/분기 예외 등).
> - **게이트 호출 존재만으로 `✓` 금지** — 범위 적정성까지 확인. 다른 항목 게이트를 추정 복사 금지.
...
4. 외부 입력 어노테이션 파라미터가 **하나도 없으면 제외**(식별자 미수용 = IDOR 대상 아님).
...
인증 인터셉터/시큐리티가 **인증 대상에서 제외한 경로**의 진입점을 표시한다. 인증 게이트를 거치지 않는 진입점일수록 애플리케이션 레벨 소유권 검증이 더 중요한데(인증이 없으니까), 이들이 `❓`로 방치되어 후보 승격이 누락되는 것을 막기 위한 우선순위 신호다.
...
3. `auth_label()` — 각 진입점 경로(verb 제거·선행 슬래시 정규화)가 제외 패턴에 매칭되면 `제외`, 아니면 `적용`, 패턴 미수집 시 `—`.

**phase1.md 정책과의 연결**: `auth=제외` + 보호 대상 리소스 접근 진입점은 phase1.md의 *"인증 게이트 미경유 진입점은 ❓로 종결 금지"* 규칙에 따라 service Read로 ✓/❌ 확정하고, 확정 불가 시 보수적으로 후보(`IDOR_GATE_UNVERIFIED`)로 승격한다. **인증 부재 자체는 안전 근거가 아니다.**

## 한계와 확장

- **controller-scan·auth 컬럼은 Spring(Java/Kotlin) 전용**이다. 어노테이션·`excludePathPatterns` 정규식 기반이므로 Python(Django/Flask/FastAPI)·Node.js(Express)·Rails·Go 프로젝트는 **taint 모드만** 동작하고, `auth` 컬럼은 `—`로 남는다. 이 경우 판정은 phase1.md 정책(인증 미경유 진입점 ❓ 종결 금지)이 백스톱이며, 에이전트가 라우팅/시큐리티 설정을 직접 Read해 판정한다.
...
- `scanners/idor-scanner/phase1.md` — 3단계 분류, 소유권게이트 채움 규칙, 인증 미경유 ❓ 종결 금지 정책, 프레임워크 확장 메타 원칙.
