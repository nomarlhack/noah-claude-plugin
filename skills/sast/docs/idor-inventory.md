...
## 신원 폴백 고정밀 룰 + 등록 게이트 (IDOR-4류 누락 방지)

인벤토리 전수 열거에 더해, **클라이언트 입력 ↔ 세션 입력의 신원 폴백**을 고정밀로 후보화하고 등록을 강제하는 2중 장치:

1. **semgrep 룰 `noah-{kotlin,java}-idor-session-identity-override`** (`scanners/idor-scanner/rules/pattern.{kotlin,java}.yaml`): 신원 식별자가 `@RequestParam`(클라)와 `@RequestHeader`(세션)의 null-폴백(Kotlin elvis `?:` / Java 삼항·`Optional.orElse`)으로 해석되는 코드를 매치한다. 클라가 대상 신원을 덮어쓸 수 있는 고정밀 IDOR 시그널(무작위 elvis 오탐 0 — 클라↔세션 혼합 폴백만). 실측: product-service 8개 컨트롤러 14곳 적발(원래 전수 누락분).
2. **등록 게이트 `_session_override_audit`** (`tools/phase1_review_assert.py`): 위 룰의 매치(locindex)가 master-list 후보로 **등록(또는 명시 폐기)됐는지** 검증한다. 미등록 매치는 예산 압박 등으로 조용히 사라진 FN으로 보고 exit 7로 차단(판정은 상태 무관, 룰 매치 존재 기준). `①(발화)+②(열거)+③(등록 강제)`가 맞물려 IDOR-4류 누락이 메커니즘적으로 재발 방지된다.

**정책**(`scanners/idor-scanner/phase1.md`): session-override 매치는 전수 등록하고, `implicit-bind`/`[제외]` 행은 file:line 증거를 인용한다(라벨만 뒤집기 금지).
