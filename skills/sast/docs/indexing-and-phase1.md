...
방어(게이트) 유무를 확인할 때 sink의 **실제 효과**도 함께 봅니다(영향축 대칭 — `guidelines-phase1.md §9-A` / `decision-framework §2-D 원칙 4`): 조회형이면 응답으로 직렬화되는 구조의 노출 필드를, 변경·부수효과형이면 일으키는 상태 변화·부수 효과를 근거로 영향을 적습니다. 엔드포인트 의미·형제 취약점·worst-case로 외삽하지 않고, 정적 확인 불가분은 "효과 미확인"으로 표기합니다(과장도, 미확인을 "안전"으로 닫는 과소평가(FN)도 금지). 과장 예: 조회형 sink의 응답 구조(직렬화 필드)를 확인하지 않고 worst-case PII로 외삽하는 것.
