...
semgrep은 49개 스캐너의 룰을 한꺼번에 실행하고, 각 매칭 결과(파일 경로·라인 번호·룰 ID)를 스캐너별로 두 가지 형태로 저장합니다.

- **`<scanner>.json`** — 룰 ID별 `[파일:라인, ...]` 목록 (평평한 인덱스)
- **`<scanner>.locindex.json`** — 위치별 `{tier, rule_ids}` 목록. 같은 파일:라인에 여러 룰이 매치되면 1개로 합치고 신뢰도가 높은 tier(`taint > ast > generic`)로 승격합니다. Phase 1이 이 파일을 분석 입력으로 사용합니다.

`locindex.json`은 수만 줄에 달할 수 있어 에이전트가 직접 읽지 않습니다. 대신 `locindex_summary.py`가 이를 **파일별 1줄 요약**으로 변환해 Phase 1 에이전트에게 전달합니다(전 스캐너 2,000줄 이내 보장).
...
| 스캐너 | 판별 방법 |
|---|---|
| idor | taint 룰(java/kotlin/python/js, 외부 입력 → DB 조회·수정·삭제 + 소유권 게이트 부재) + **`idor_inventory.py` 두 모드**(taint 원장 + 컨트롤러 전수 스캔) + 게이트 범위 적정성 확인 + 관례 일탈 신호 |
...
| 스캐너 | taint 룰 수 |
|---|---|
| sqli | 8 (go·java·js·kotlin·php·python·ruby + 신규) |
| ssrf · path-traversal | 7 · 7 (kotlin·ruby 추가) |
| idor | 4 (java·kotlin·python·js 신규) |
| open-redirect | 6 (java·js·php·python·ruby·kotlin 신규) |
| crlf-injection | 6 (java·js·php·python·ruby·kotlin 신규) |
| command-injection | 6 (go·java·js·kotlin·python·ruby) |
| xxe · ldap-injection · xpath-injection | 3 · 2 · 2 |
| nosqli · deserialization · code-injection | 3 · 3 · 3 |
| log-injection | 3 (java·js·python 신규) |
| ssti · file-upload | 4 · 0 |

**D. 공통 층과 패턴/generic 룰로 검토 (나머지 약 30개)**
...
`tls` · `security-headers` · `cookie-security` · `springboot-hardening` · `http-smuggling` · `sourcemap` · `subdomain-takeover` · `host-header` · `http-method-tampering` · `csv-injection` · `jwt` · `oauth` · `saml` · `redos` · `prototype-pollution` · `graphql` · `websocket` · `xslt-injection` · `soapaction-spoofing` · `android-deeplink` · `zipslip` · `pdf-generation` · `prompt-injection` · `system-prompt-leakage` · `insecure-output-handling` · `unbounded-consumption` · `css-injection` · `hardcoded-secrets` · `log-injection`
...
| 49 | log-injection-scanner | Log Injection / Sensitive Info Logging | secrets-logging |
